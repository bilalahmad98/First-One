...
{
  	int c_socket, x;							// define your variables here
  ...
  if (argc != 2)
	...
	c_socket = socket(AF_INET, SOCK_STREAM, 0);				// Write your code here
	if (c_socket < 0)
	{
	...
	if (connect(c_socket, (struct sockaddr *) &srv_ad, srv_size) < 0)
	{
